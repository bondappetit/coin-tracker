...
import {PANKCAKESWAP_HOURLY_DATA_TABLE, PANKCAKESWAP_SWAPS_TABLE} from "../trackers/pancakeswap";
...
export async function up(knex: Knex): Promise<void> {
    ...
    await knex.schema.createTable(PANKCAKESWAP_SWAPS_TABLE, table => {
        table.string('id').primary();
        table.timestamp('timestamp');
        table.string('token0Symbol');
        table.string('token1Symbol');
    ...
    await knex.schema.createTable(PANKCAKESWAP_HOURLY_DATA_TABLE, table => {
        table.timestamp('timestamp');
        table.string('pairId');
        table.string('token0Symbol');
        table.string('token1Symbol');
